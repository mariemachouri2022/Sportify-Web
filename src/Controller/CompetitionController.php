...
class CompetitionController extends AbstractController
{
    ...
    $equipe2Members = $entityManager->getRepository(EquipeMembers::class)->findBy(['equipeId' => $competition->getEquipe2()->getIdEquipe()]);
        ...
        foreach ($equipe1Members as $equipe1Member) {
            $user = $entityManager->getRepository(Utilisateur::class)->find($equipe1Member->getUserId());
            if ($user) {
                $equipe1Users[] = $user;
            }
        }
        ...
        foreach ($equipe2Members as $equipe2Member) {
            $user = $entityManager->getRepository(Utilisateur::class)->find($equipe2Member->getUserId());
            if ($user) {
                $equipe2Users[] = $user;
            }
        }
...
public function createCharge(FlashyNotifier $flashy,Request $request)
{
    ...
    public function joinTeam(CompetitionRepository $competRepo ,EntityManagerInterface $entityManager,UtilisateurRepository $userrepo, Request $request, int $equipeId, int $competitionId): Response
{
    ...
    public function TeamsClient(CompetitionRepository $competitionRepository): Response
    {
        return $this->render('competition/TeamsClient.html.twig', [
            'competitions' => $competitionRepository->findByType("TEAMS"),
        ]);
    }
